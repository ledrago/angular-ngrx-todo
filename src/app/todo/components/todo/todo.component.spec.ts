...
describe('TodoComponent', () => {
  ...
  it('Get Todo action should load data from api', () => {
    let nextState: TodoState = {
      TodoList: [{id: 1, title: "title one", description: "description 1", done: false}],
      TodoError: null
    }
    store.setState({todos: nextState});
    ...
    const payload = [{id: 2, title: "title two", description: "description 2", done: true}];
    todoServiceSpy.getTodos.and.returnValue(of(payload));

    const effects = new TodoEffects(actions, todoServiceSpy);
    const expected = cold('b', {b: {type: TodoActions.SuccessGetTodos.type, payload}});
